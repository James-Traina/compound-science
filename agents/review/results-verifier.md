...
description: >-
  Audits the accuracy of reported results by tracing the chain from code output to tables, figures, and text. Checks that numbers in tables match code output, significance stars are correct, sample sizes are consistent, and results described in text match the tables. Use after generating tables or figures, before submission, or when verifying that reported results accurately reflect the underlying analysis.
