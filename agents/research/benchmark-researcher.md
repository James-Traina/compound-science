...
description: >-
  Researches empirical benchmarks, calibration targets, stylized facts, and reference parameter values from the economics literature. Use when calibrating a model and needing standard parameter values, looking for empirical moments to target, comparing calibration choices across papers, finding stylized facts a model should match, or researching consensus estimates for key elasticities or rates.
