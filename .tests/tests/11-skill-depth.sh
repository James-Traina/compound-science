...
# v0.8: SKILLS reduced from 27 to 20 (7 skills merged into workflow/chain skills)
# Depth/quality checks exclude stub skills; naming checks updated for new patterns.
source "$(dirname "$0")/../lib/assert.sh"
source "$(dirname "$0")/../lib/fixtures.sh"

SKILLS_DIR="$PLUGIN_DIR/skills"

# ORIGINAL_SKILLS sourced from fixtures.sh (10 domain knowledge skills)

group "Content Depth"

# 1: All 10 original skills have >=100 lines
all_long=true
...
if $all_long; then pass "all 10 original skills have >= 100 lines"; fi

# 2: All 10 original skills have >=3 ## section headers
all_headers=true
for skill in "${ORIGINAL_SKILLS[@]}"; do
...
if $all_headers; then pass "all 10 original skills have >= 3 section headers"; fi

# 3: All 10 original skills have code examples
all_code=true
for skill in "${ORIGINAL_SKILLS[@]}"; do
...
if $all_code; then pass "all 10 original skills have code examples"; fi

# 4: All 10 original skills have >=500 words
all_words=true
for skill in "${ORIGINAL_SKILLS[@]}"; do
  f="$SKILLS_DIR/$skill/SKILL.md"
...
if $all_words; then pass "all 10 original skills have >= 500 words"; fi

group "Trigger Quality"

# 5: All skill descriptions have trigger keywords (>=50 chars in first non-frontmatter paragraph)
# Only check original skills — stubs and wrappers have short descriptions by design
all_desc=true
for skill in "${ORIGINAL_SKILLS[@]}"; do
...
if $all_desc; then pass "all original skills have substantial trigger descriptions"; fi

# 6: No two original skills share >8 identical significant words in body opener
...
# Only checks original 17 skills; migrated skills are tested separately.
...
# Only check original 17 skills
original = {
  'causal-inference','causal-ml','game-theory',
  'identification-proofs','bayesian-estimation',
  'reproducible-pipelines','structural-modeling',
  'submission-guide','empirical-playbook',
  'publication-output'
}
skill_words = {}
for skill in os.listdir(skills_dir):
    if skill not in original: continue
...
for i, a in enumerate(names):
    for b in names[i+1:]:
        ...
        if len(shared) > 8:
            ...
            sys.exit(1)
" 2>&1); then
  pass "no two original skills share >8 identical trigger words"
else
...
# 7-20: Each original skill contains expected domain terms
# (migrated skills are stubs/wrappers/chains — tested for references in 09)
get_skill_terms() {
  case "$1" in
...
# 21: Original skills reference at least one agent name
AGENT_PATTERN="econometric-reviewer|mathematical-prover|numerical-auditor|identification-critic|journal-referee|literature-scout|methods-explorer|data-detective|reproducibility-auditor|workflow-coordinator"
...
if [ "$ref_count" -eq 10 ]; then
  pass "all original skills reference at least one agent ($ref_count/10)"
elif [ "$ref_count" -ge 8 ]; then
  should_fix "all original skills reference an agent" "$ref_count/10 skills reference agents"
else
  must_fix "all original skills reference an agent" "only $ref_count/10 skills reference agents"
fi

# 22: No original skill exceeds 2000 lines (bloat guard)
all_slim=true
for skill in "${ORIGINAL_SKILLS[@]}"; do
  f="$SKILLS_DIR/$skill/SKILL.md"
  lines=0; [ -f "$f" ] && lines=$(wc -l < "$f" | tr -d ' ')
...
if $all_slim; then pass "no original skill exceeds 2000 lines"; fi

# 23: Original skill directories contain exactly 1 file (SKILL.md) or have references/ subdirectory
all_single=true
for skill in "${ORIGINAL_SKILLS[@]}"; do
...
if $all_single; then pass "all original skill directories contain exactly 1 file"; fi

# 24: All original skill names match word-word kebab-case
all_kebab=true
for skill in "${ORIGINAL_SKILLS[@]}"; do
...
if $all_kebab; then pass "all original skill names match word-word kebab-case"; fi
